...
class Autoencoder(object):
    ...
    def _initialize_weights(self):
		...
		all_weights['w4'] = tf.get_variable("w4", shape=[self.n_hidden3, self.n_hidden4],
            initializer=tf.contrib.layers.xavier_initializer())
        all_weights['b4'] = tf.Variable(tf.zeros([self.n_hidden4], dtype=tf.float32))
		all_weights['w5'] = tf.get_variable("w5", shape=[self.n_hidden4, self.n_hidden5],
            initializer=tf.contrib.layers.xavier_initializer())
        all_weights['b5'] = tf.Variable(tf.zeros([self.n_hidden5], dtype=tf.float32))
        all_weights['w6'] = tf.Variable(tf.zeros([self.n_hidden5, self.n_input], dtype=tf.float32))
        all_weights['b6'] = tf.Variable(tf.zeros([self.n_input], dtype=tf.float32))
        return all_weights
